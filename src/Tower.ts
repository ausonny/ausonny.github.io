...
function numberOfTowerType(type: string) {
  let ret = 0;
...
function maxTowersByType(type: string) {
  const ret = 1;
  switch (type) {
    case 'Gun':
    case 'Missile':
...
function availableTowersByType(type: string) {
...
class Tower {
  ...
  button4Element: HTMLElement;

  button5Element: HTMLElement;

  button6Element: HTMLElement;

  showRangeElement: HTMLInputElement;
  ...
  pebbleDamageUpgrade: Upgrade;

  rockDamageUpgrade: Upgrade;

  boulderDamageUpgrade: Upgrade;

  equipmentPhrase: string;

  bonusPerLevelBoughtUpgrade: Upgrade;
  ...
  AttackValue() {
    let AttackPerLevel = this.baseAttack * 1.1 ** (this.upgradeLevel + this.pebbleDamageUpgrade.bought);
    AttackPerLevel *= 2 ** (this.rockDamageUpgrade.bought + this.boulderDamageUpgrade.bought);
  ...
  PoisonAttack() {
    ...
    AttackPerLevel *= 1.1 ** this.upgradeLevel;
...
class Enemy extends movingObject {
  ...
  act() {
    ...
    if (this.type === 'Boss') {
      if (gameData.world.currentWave > gameData.world.highestWaveCompleted) {
        dustGained = new JBDecimal(10 * 1.05 ** gameData.world.currentWave);
        dustGained = dustGained.multiply(2 ** (gameData.world.currentTier - 1));
  ...
  beTargetedByWeapon(damagePassed: JBDecimal, attackerPos: Vector, DefenseDamage: number) {
