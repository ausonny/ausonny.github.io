...
class Bullet extends movingObject {
  ...
  constructor(pos: Vector, target: Vector, damage: JBDecimal, defenseDamage: number, crit: boolean) {
    const tList = [];
    tList.push(target);
...
class Tower {
  ...
  baseGunAttack: number;

  ticksToNextBullet: number;

  baseRange: number;

  baseTicksPerShot: number;
  ...
  gunCostMultiplier: number;

  gunUpgradeCost: number;

  gunUpgradeCostMultiplier: number;

  baseSlowCost: number;

  slowCostMultiplier: number;

  slowUpgradeCost: number;

  slowUpgradeCostMultiplier: number;

  basePoisonCost: number;

  poisonCostMultiplier: number;

  poisonUpgradeCost: number;

  poisonUpgradeCostMultiplier: number;
  ...
  button1Element: HTMLElement;

  button1AutoElement: HTMLElement;

  button2Element: HTMLElement;

  button3Element: HTMLElement;

  deleteElement: HTMLElement;

  towerRow: HTMLElement;
        ...
        if (enemiesInRange.length > 0) {
          if (gameData.tactics.fastest) {
            ...
            enemiesInRange.sort((a, b) =>
              a.expectedHitsRemaining(this.GunAttack()) < b.expectedHitsRemaining(this.GunAttack()) ? 1 : -1
            );
    ...
    if (this.type === 'Slow') {
      if (gameData.enemies.length > 0) {
        gameData.enemies.forEach((e) => {
          const len = e.pos.getLengthFromAnotherVector(this.pos);
          if (len <= this.Range()) {
            if (e.slowed < this.SlowEffect()) e.slowed = this.SlowEffect();
          }
        });
      }
    }
  ...
  draw() {
    ...
    switch (this.type) {
      case 'Gun': {
        ...
        const chk = document.getElementById('showGunArea') as HTMLInputElement;
        if (chk.checked) {
          display.DrawCircle(this.pos, 0.02, this.Range(), 'red');
        }
        document.getElementById(
          `Tower${this.index.toString()}Info`
        ).innerHTML += ` Attack: ${this.GunAttack().ToString()}`;
      ...
      case 'Slow': {
        ...
        const chk = document.getElementById('showSlowArea') as HTMLInputElement;
        if (chk.checked) {
          display.DrawCircle(this.pos, 0.02, this.Range(), 'blue');
        }

        document.getElementById(`Tower${this.index.toString()}Info`).innerHTML += ` Slow: ${(
          1 /
          (1 + this.SlowEffect())
        ).toFixed(3)}`;
      ...
      case 'Poison': {
        ...
        const chk = document.getElementById('showPoisonArea') as HTMLInputElement;
        if (chk.checked) {
          display.DrawCircle(this.pos, 0.02, this.Range(), 'green');
        }
        document.getElementById(
          `Tower${this.index.toString()}Info`
        ).innerHTML += ` Poison: ${this.PoisonAttack().ToString()}`;
  ...
  GunAttack() {
    ...
    AttackPerLevel *= 2 ** (gameData.rockUpgrades[1].bought + gameData.boulderUpgrades[4].bought);

    if (gameData.upgrades[6].bought > 0) {
      const bonusPer = 1 + gameData.upgrades[6].bought / 100;
      AttackPerLevel *= bonusPer ** this.bought;
    }
  ...
  PoisonAttack() {
    ...
    AttackPerLevel *= 2 ** (gameData.rockUpgrades[13].bought + gameData.boulderUpgrades[9].bought);
  ...
  // eslint-disable-next-line class-methods-use-this
  skillDeterioration() {
    return (0.999 - (gameData.world.currentTier - 1) / 1000) ** gameData.world.currentWave;
  }
...
class Enemy extends movingObject {
  ...
  act() {
        ...
        gameData.enemies.forEach((e) => {
          if (e.type !== 'Cleric') {
            ...
            if (e.pos.getLengthFromAnotherVector(me.pos) < healRange) {
              e.damagetaken = e.damagetaken.subtract(
                me.CurrentHitPoints().divide(2000).multiply(gameData.world.currentTickLength)
              );
    ...
    const UpgradeBonus = 0.1 * 2 ** gameData.rockUpgrades[2].bought;
    const lootBonus = (1 + UpgradeBonus) ** gameData.upgrades[1].bought;
  ...
  beTargetedbyGun(attacker: Tower) {
  ...
  expectedHitsRemaining(damage: JBDecimal) {
