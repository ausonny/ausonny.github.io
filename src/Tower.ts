...
class Building extends Purchasable {
  ...
  // baseWoodStorage: number;

  // baseStoneStorage: number;

  // baseRedResearchStorage: number;
  ...
  netWoodPerSec: JBDecimal;

  netStonePerSec: JBDecimal;

  netArrowPerSec: JBDecimal;

  netRedResearchPerSec: JBDecimal;
  ...
  slowToValue() {
    ...
    let ret = (1 - this.baseSlow / 20) ** (this.bought * gameData.challenges[4].completed);
    ret /= skillDeterioration();
    return ret;
  }
  ...
  poisonAttackValue() {
    ...
    ret = ret.multiply((1 + gameData.pebbleUpgrades[8].bought / 100) ** this.bought);

    ret = ret.multiply(1.1 ** gameData.researches[6].bought);
    ret = ret.multiply(2 ** gameData.pebbleUpgrades[3].bought);
    return ret;
  }
  ...
  redResearchProductionPerSec() {
    ...
    ret *= 1.1 ** gameData.powderUpgrades[13].bought;
    ret *= this.powderUpgrade7Bonus();
    ret *= this.powderUpgrade9Bonus();
    ret *= (1 + gameData.powderUpgrades[18].bought / 100) ** this.bought;
    if (gameData.equipment.length > 0) {
      gameData.equipment[0].abilities.forEach((a) => {
  ...
  woodProductionPerSec() {
    ...
    ret *= 1.1 ** (gameData.powderUpgrades[2].bought + gameData.researches[4].bought);
    ret *= this.powderUpgrade7Bonus();
    ret *= this.powderUpgrade9Bonus();
    ret *= (1 + gameData.powderUpgrades[14].bought / 100) ** boughtToUse;
    if (gameData.equipment.length > 0) {
      gameData.equipment[0].abilities.forEach((a) => {
        if (a.name === this.equipmentPhrase) {
          ret *= 1 + a.levels / 10;
  ...
  stoneProductionPerSec() {
    ...
    let ret = boughtToUse * this.baseProductionPerSec;
    ret *= 1.1 ** (gameData.powderUpgrades[12].bought + gameData.researches[5].bought);
    ret *= this.powderUpgrade7Bonus();
    ret *= this.powderUpgrade9Bonus();
    ret *= (1 + gameData.powderUpgrades[15].bought / 100) ** boughtToUse;
    if (gameData.equipment.length > 0) {
      gameData.equipment[0].abilities.forEach((a) => {
        if (a.name === this.equipmentPhrase) {
          ret *= 1 + a.levels / 10;
  ...
  arrowProductionPerSec() {
    let ret = this.bought * this.baseProductionPerSec;
    ret *= 1.1 ** (gameData.powderUpgrades[6].bought + gameData.researches[7].bought);
    ret *= this.powderUpgrade7Bonus();
    ret *= this.powderUpgrade9Bonus();
    ret *= (1 + gameData.powderUpgrades[19].bought / 100) ** this.bought;
    if (gameData.equipment.length > 0) {
      gameData.equipment[0].abilities.forEach((a) => {
        if (a.name === this.equipmentPhrase) {
          ret *= 1 + a.levels / 10;
  ...
  arrowAttackValue() {
    let AttackPerLevel = this.baseArrowAttack * 1.1 ** (gameData.researches[1].bought + gameData.powderUpgrades[0].bought);
    AttackPerLevel *= 2 ** gameData.pebbleUpgrades[1].bought;
    AttackPerLevel *= (1 + (gameData.powderUpgrades[8].bought + gameData.pebbleUpgrades[14].bought) / 100) ** this.bought;
  ...
  catapultAttackValue() {
    let AttackPerLevel = this.baseCatapultAttack * 1.1 ** (gameData.researches[8].bought + gameData.powderUpgrades[16].bought);
    AttackPerLevel *= 2 ** gameData.pebbleUpgrades[2].bought;
    AttackPerLevel *= (1 + (gameData.powderUpgrades[17].bought + gameData.pebbleUpgrades[15].bought) / 100) ** this.bought;
    // AttackPerLevel *= this.bought;
...
class Enemy extends movingObject {
  ...
  act() {
    ...
    if (gameData.world.currentWave > gameData.world.highestWaveCompleted) {
      essenceGained = new JBDecimal(1.05 ** gameData.world.currentWave);
