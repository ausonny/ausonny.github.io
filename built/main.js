...
function processStuff(ticks) {
    ...
    if (housingAvailable.greaterThan(0)) {
        const peoplegrowth = housingAvailable
            ...
            .multiply(Math.pow(1.1, gameData.powderUpgrades[10].bought))
...
function updateGUI() {
    ...
    if (powderFromPrestige().greaterThanOrEqualTo(1)) {
        ...
        display.displayTextArrayFromString(`Prestige for ${powderFromPrestige().ToString()} powder<br />Current: ${getCurrentPowderRate().ToString()} /hr<br />Best: ${gameData.stats.bestPrestige1Rate.ToString()}/hr`, new Vector(CANVAS_SIZE * 0.2, CANVAS_SIZE * 0.05), 'white', 'center', 'middle');
    }
    if (pebbleFromPrestige().greaterThanOrEqualTo(1)) {
        ...
        display.displayTextArrayFromString(`Ascend for ${pebbleFromPrestige().ToString()} pebbles<br />Current: ${getCurrentPebblesRate().ToString()} /hr<br />Best: ${gameData.stats.bestPrestige2Rate.ToString()}/hr`, new Vector(CANVAS_SIZE * 0.4, CANVAS_SIZE * 0.05), 'white', 'center', 'middle');
    }
    if (rockFromPrestige().greaterThanOrEqualTo(1)) {
        ...
        display.displayTextArrayFromString(`Transform for ${rockFromPrestige().ToString()} rocks<br />Current: ${getCurrentRocksRate().ToString()} /hr<br />Best: ${gameData.stats.bestPrestige3Rate.ToString()}/hr`, new Vector(CANVAS_SIZE * 0.6, CANVAS_SIZE * 0.05), 'white', 'center', 'middle');
    }
    ...
    if (UIElementActive.slice(0, 1) === 'R') {
        ...
        display.displayTextArrayFromString(`Buy Red<br />Research Lab<br />People: 10<br />Wood: ${RED_RESEARCH_LAB_WOOD_COST}<br />Stone: ${RED_RESEARCH_LAB_STONE_COST}`, new Vector(CANVAS_SIZE * 0.55, CANVAS_SIZE * 0.95), 'white', 'center', 'middle');
        return;
    }
